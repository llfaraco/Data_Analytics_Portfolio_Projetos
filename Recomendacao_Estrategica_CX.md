...
1. Risco Máximo na Jornada de Onboarding: O maior risco de Churn (maior pontuação no modelo) está concentrado em clientes que estão inativos (dias_inatividade > 45) E que tiveram 3 ou mais interações com o suporte logo após a primeira compra (falha de Logística/Atendimento).
2. Impacto Operacional: O volume de tickets de suporte (Métrica Operacional) apresentou um alto coeficiente no modelo, comprovando que falhas em Logística ou Atendimento são o principal gatilho de evasão para essa base específica.
3. Prioridade: A lista de ID de Clientes de Alto Risco foi gerada (acessível na saída do script Python) e deve ser a prioridade da equipe de CX.

4. Recomendação de Ação Proativa (CX/Produto)
